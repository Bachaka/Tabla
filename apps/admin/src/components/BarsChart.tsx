/**
 * components/BarsChart — histogramme SVG au style du prototype.
 */
export function BarsChart({
  ...
  if (data.length === 0) {
    return <p style={{ color: "var(--ink-mute)", fontSize: 13 }}>Aucune donnée.</p>;
  }
  const w = 600;
  ...
  return (
    <svg className="chart-svg" viewBox={`0 0 ${w} ${height}`} preserveAspectRatio="none">
      ...
      {data.map((d, i) => {
        ...
        return (
          <g key={d.m}>
            <rect x={x} y={y} width={bw} height={h} rx="2" fill={color} opacity="0.86" />
            <text
              x={x + bw / 2}
              y={height - 10}
              fontSize="10"
